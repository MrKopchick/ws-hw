...
const endGame = (io, room) => {
    ...
    const allResults = [...finishedUsers, ...unfinishedUsers].sort((a, b) => (a.finishedAt || Infinity) - (b.finishedAt || Infinity));
...
const handleReadyToggle = (io, socket, rooms) => () => {
...
const handleProgressUpdate = (io, socket, rooms) => (progress) => {
    const room = findRoomBySocket(rooms, socket);
    if (!room || !room.gameStarted) {
        return;
    }
...
const handlePlayerFinished = (io, socket, rooms) => () => {
    const room = findRoomBySocket(rooms, socket);
    if (!room || !room.gameStarted) {
        return;
    }
...
const removeRoomIfEmpty = (io, rooms, room) => {
...
const handlePlayerDisconnect = (io, socket, rooms) => () => {
    const room = findRoomBySocket(rooms, socket);
    if (!room) {
        return;
    }
...
const handleRequestRoomReset = (io, socket, rooms) => () => {
    const room = findRoomBySocket(rooms, socket);
    if (!room) {
        return;
    }
...
export { checkAllReady, endGame, handlePlayerDisconnect, handlePlayerFinished, handleProgressUpdate, handleReadyToggle, handleRequestRoomReset };

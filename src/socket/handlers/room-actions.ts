...
import { type JoinRoomParameters, type Room } from "../../types.js";
...
const joinRoom = async ({ io, roomName, rooms, socket, username }: JoinRoomParameters): Promise<void> => {
...
const leaveRoom = (io: Server, socket: Socket, rooms: Room[]): void => {
...
const handlePostDisconnectGameState = (io: Server, room: Room, rooms: Room[]): void => {
    if (room.gameStarted) {
...
const cleanupRoom = (io: Server, room: Room, rooms: Room[]): void => {
    if (room.timer) {
